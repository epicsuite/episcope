...
from episcope.library.viz.data_source import StructureSource, PeakTrackSource, PointTrackSource
from episcope.library.viz.display import TubeDisplay, LowerGaussianContourDisplay, UpperGaussianContourDisplay, DelaunayDisplay
...
class Visualization:
    ...
    def set_chromosome(self, chromosome: str, experiment: str, timestep: str):
        ...
        for structure_point in structure:
            for i, coord in enumerate(("x", "y", "z")):
                self._splines[coord].AddPoint(structure_point["index"], structure_point["position"][i])
    ...
    def add_structure_display(self, display_type: str, point_spacing: int):
        structure = self._source.get_structure(self._chromosome, self._experiment, self._timestep)

        structure_indices = list(map(lambda p: p["index"], structure))
        ...
        representation = simple.Show(display.output, self.render_view, "GeometryRepresentation")
    ...
    def add_peak_display(self, track_name: str, display_type: str, point_spacing: int):
        track = self._source.get_peak_track(self._chromosome, self._experiment, self._timestep, track_name)
    ...
    def add_point_display(self, track_name: str, display_type: str, point_spacing: int):
        track = self._source.get_point_track(self._chromosome, self._experiment, self._timestep, track_name)
    ...
    def _update_displays(self):
        for display_id, display_meta in self._displays.items():
            track_name = display_meta["track_name"]
            track_type = display_meta["track_type"]
            enabled = display_meta["enabled"]
            
            track_data = None
            try:
                if track_type == "peak":
                    track_data = self._source.get_peak_track(self._chromosome, self._experiment, self._timestep)
                else:
                    track_data = self._source.get_point_track(self._chromosome, self._experiment, self._timestep)
